...
@property (strong, nonatomic) UICollectionView *opponentsCollectionView;
...
- (void)viewDidLoad {
  ...
  //  [self configureGUI];
    ...
    self.opponentsCollectionView = [[UICollectionView alloc] initWithFrame:self.view.bounds];
    self.opponentsCollectionView.delegate = self;
    self.opponentsCollectionView.dataSource = self;
    [self.view addSubview:self.opponentsCollectionView];
    ...
    _isOffer = ([self currentUserID] == self.session.initiatorID);
    ...
    if (!self.isOffer) {
        [self.users addObject:[self currentUserID]];
    }
...
- (UIView *)videoViewWithOpponentID:(NSNumber *)opponentID {
    ...
    if ([self currentUserID] == opponentID) {//Local preview
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSNumber *userID = self.users[indexPath.row];
    
    [cell setVideoView:[self videoViewWithOpponentID:userID]];
...
- (NSIndexPath *)indexPathAtUserID:(NSNumber *)userID {
 ...
 //   QBUUser *user = [UsersDataSource.instance userWithID:userID];
    NSUInteger idx = [self.users indexOfObject:userID];
