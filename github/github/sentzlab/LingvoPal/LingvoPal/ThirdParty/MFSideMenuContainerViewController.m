...
@interface MFSideMenuContainerViewController () <PortalSessionDelegate>
...
@property (nonatomic, strong) NSTimer *setOnlineTimer;
...
- (id) init {
    ...
    if(self) {
        ...
        self.greyView = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
        self.greyView.backgroundColor = [UIColor colorWithWhite:.0f alpha:.5f];
    }
    return self;
}

- (void)dealloc {
    [Helper deinitializeTimer:self.setOnlineTimer];
    [[NSNotificationCenter defaultCenter] removeObserver:self name:kOpenOrdersNotification object:nil];
...
- (void)viewDidLoad {
 ...
 //   OrderViewController *orderViewController = [[OrderViewController alloc] init];
    HistoryViewController *historyViewController = [[HistoryViewController alloc] init];//[[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"HistoryViewController"];
    
 //   ProfileViewController *profileViewController = [[ProfileViewController alloc] init];
    MyOrdersViewController *myOrdersViewController = [[MyOrdersViewController alloc] init];
    
    TranslatorHistoryViewController *translatorHistoryViewController = [[TranslatorHistoryViewController alloc] init];
    
 //   TranslatorReviewViewController *translatorReviewViewController = [[TranslatorReviewViewController alloc] init];
    PaymentViewConytoller *paymentViewConytoller = [[PaymentViewConytoller alloc] init];
    TranslatorSettingsViewController *translatorSettingsViewController = [[TranslatorSettingsViewController alloc] init];
    
    LeftMenuViewController *leftMenuViewController = [[LeftMenuViewController alloc] init];
    leftMenuViewController.findInterpreterViewController = [[FindInterpreterViewController alloc] init];
 //   leftMenuViewController.orderViewController = orderViewController;
    leftMenuViewController.historyViewController = historyViewController;
 //   leftMenuViewController.profileViewController = profileViewController;
    leftMenuViewController.paymentViewConytoller = paymentViewConytoller;

    leftMenuViewController.myOrdersViewController = myOrdersViewController;
    leftMenuViewController.translatorHistoryViewController = translatorHistoryViewController;
 //   leftMenuViewController.translatorReviewViewController = translatorReviewViewController;
    leftMenuViewController.translatorSettingsViewController = translatorSettingsViewController;
    ...
    NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    NSString *transId =  [defaults objectForKey:kTranslatorId];
    BOOL isTranlator = (transId != nil && ![transId isEqualToString:@"-1"]);
    
    if (isTranlator) {
        [self setCenterViewController:leftMenuViewController.myOrdersViewController];
    } else {
        [self setCenterViewController:leftMenuViewController.findInterpreterViewController];
    }
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(openOrdersNotificationAction:) name:kOpenOrdersNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(acceptOrderNotificationAction:) name:kAcceptOrderNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(acceptTranslatorNotificationAction:) name:kAcceptTranslatorNotification object:nil];
    
    self.setOnlineTimer = [NSTimer scheduledTimerWithTimeInterval:3.0f target:self
                                                    selector:@selector(setOnline) userInfo:nil repeats:YES];
...
-(void) openOrdersNotificationAction:(NSNotification *) notification{
    
   // NSDictionary* userInfo = notification.object;
    [self.leftMenuViewController openOrders];
}

-(void) acceptOrderNotificationAction:(NSNotification *) notification{
    
  //  NSDictionary* userInfo = notification.object;
    
}

-(void) acceptTranslatorNotificationAction:(NSNotification *) notification{
...
//    
 ...
 //   NSString *stream_id = [notifs objectAtIndex:4];
    
//    VideoViewController *viewController = [[VideoViewController alloc] init];
//    viewController.streamId = stream_id;
...
- (void)setLeftMenuViewController:(LeftMenuViewController *)leftSideMenuViewController {
...
- (void)openLeftSideMenuCompletion:(void (^)(void))completion {
    ...
    [self.leftMenuViewController leftMenuOpened];
    [((UIViewController *) self.centerViewController).view addSubview:self.greyView];
...
- (void)closeSideMenuCompletion:(void (^)(void))completion {
    ...
    [self.leftMenuViewController leftMenuClosed];
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    if ([gestureRecognizer isKindOfClass:[UIPanGestureRecognizer class]]) {
        ...
        BOOL isHorizontalPanning = fabsf((float)velocity.x) > fabsf((float)velocity.y);
...
-(void) presentVideoCallParts{
    
    NSUserDefaults * defaults = [NSUserDefaults standardUserDefaults];
    
    NSString *streamId = [defaults objectForKey:kStreamId];
...
-(void) checkStreamResponse:(NSDictionary *) dict{
    ...
    CallType callType = (CallType)[[dict objectForKey:@"call_type"] integerValue];
    ...
    if (isTranslator && callType == sntzConfVideoCall) {
        [defaults setObject:[dict objectForKey:kStreamId] forKey:kStreamId];
        [self presentVideoCallParts];
    }
}
...
-(void) failedToCheckStream:(NSDictionary *) dict{
    ...
    if (errorCode == 100) {
        ...
        if ([[dict objectForKey:@"edit_review"] integerValue] == 1){
            NSString *transId = [dict objectForKey:@"tr_id"];
            
            RateViewController *viewController = [[RateViewController alloc] init];
            viewController.content = @"New screen with spent money, you are initiator and you hunged up";
...
-(void) setOnlineResponse:(NSDictionary *) dict{
    NSLog(@"setOnlineResponse %@", dict);
}

-(void) failedToSetOnline:(NSDictionary *) dict{
    NSLog(@"failedToSetOnline  %@", dict);
